...
class Gallery extends Base
{
    ...
    public function index()
    {
        ...
        $result = array();
        $result['pages'] = $this->_contentTotalPages(Db::name('BasicGallery')->where($where)->count('id'));
        if($result['pages'] && ($contentArr = Db::name('BasicGallery')->field('id,title,digest,local_url,link_url,cat_2,reads,create_at')->where($where)->order('sort desc,id desc')->limit($this->limit)->select()))
        {
    ...
    public function lists()
    {
        ...
        $result = array();
        $result['pages'] = $this->_contentTotalPages(Db::name('BasicGallery')->where($where)->count('id'));
        if($result['pages'] && ($contentArr = Db::name('BasicGallery')->field('id,title,digest,local_url,link_url,cat_2,reads,create_at')->where($where)->order('sort desc,id desc')->limit($start,$limit)->select()))
        {
            foreach($contentArr as $contentVal)
            {
                $cvArr['id'] = $contentVal['id'];
    ...
    public function details()
    {
        ...
        $result = array();
        if(!empty($where['id']) && ($contentDatas = Db::name('BasicGallery')->field('id,title,keywords,digest,detail,reads,create_at')->where($where)->find()))
        {
            ...
            GalleryService::recordContentInfo(['type'=>$type,'ctId'=>$contentDatas['id'],'ctTitle'=>$result['content']['title'],'reads'=>$contentDatas['reads']]);
        }
