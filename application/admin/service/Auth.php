...
class Auth
{
    ...
    public function handle(\think\Request $request, \Closure $next)
    {
        ...
        // 登录状态检查
        if(!defined('IS_ADMIN') || (constant('IS_ADMIN') == 2 && stripos($_SERVER['REQUEST_URI'],'wechat.php/admin') != FALSE)){
            $msg = ['code' => 0, 'msg' => '抱歉，您还没有登录获取访问权限！', 'url' => $request->domain()];
            return $request->isAjax() ? json($msg) : redirect($msg['url']);
        } else if (!empty($info['is_login']) && !self::isLogin()) {
    ...
    public static function checkPassword($password)
    {
        ...
        if (!strlen($password) >= 5) {
            return ['code' => 0, 'msg' => '密码必须大于5字符！'];
        }
        if (!preg_match(Base::$pregMatchPwdCheck, $password)) {
