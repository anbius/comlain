...
class Gallery
{
    ...
    public static function getContentCateAll()
    {
        ...
        $cateList = Db::name('BasicGalleryCate')->where($cateWhere)->order('sort desc,id asc')->column($cateField);
    ...
    public static function sameContentCate($type = 0)
    {
        ...
        while($flag)
        {
            $cate = Db::name('BasicGalleryCate')->field('id,pid')->where(['id' => $type, 'status' => '1', 'is_deleted' => '1'])->order('sort desc,id desc')->find();
            if($cate['pid'] == 0)
            {
                $flag = false;
            }else{
    ...
    public static function recordContentInfo($data)
    {
        ...
        if($data['reads'] !== ''){Db::name('BasicGallery')->where(['id'=>$insert['ct_id']])->update(['reads'=>$updateReads + 1]);}

        Db::name('LogGallerySee')->insert($insert);
    }
}