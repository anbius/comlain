...
$(function () {
    ...
    $.fn.uploadOneImage = function () {
        ...
        $(this).attr('name', name).after($tpl.data('input', this)).on('change', function () {
            ...
            if(this.value){
                var tpls = '<div class="uploadimage uploadimagemtl"><a class="layui-icon">&#x1006;</a></div>';
                var $tpls = $(tpls).attr('data-tips-image', this.value).css('backgroundImage', 'url(' + this.value + ')');
                var _that = this;
                $tpls.on('click', 'a', function (e) {
                    e.stopPropagation();
                    var $cur = $(this).parent();
